...
@implementation ElementTypeTransformer
...
{
    if (![typeName hasPrefix:@"XCUIElementType"]) {
        typeName = [NSString stringWithFormat:@"XCUIElementType%@", typeName];
    }
    [self createMapping];
    ...
    if (!type) {
        ...
        @throw [NSException exceptionWithName:@"InvalidArgumentException" reason:reason userInfo:@{}];
    }
